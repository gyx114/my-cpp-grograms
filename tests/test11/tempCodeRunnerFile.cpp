...
public:
    ...
    point<clss, v> get_low() const
    {
        ...
        point<clss, v> min_point = *vertices[0];
        for (int i = 0; i < v; i++)
        {
            for (auto *vert : vertices)
            {
                if (vert->coords[i] < min_point.coords[i])
                {
                    min_point.coords[i] = vert->coords[i];
                }
            }
        }
        return min_point;
    ...
    point<clss, v> get_high() const
    {
        ...
        point<clss, v> max_point = *vertices[0];
        for (int i = 0; i < v; i++)
        {
            for (auto *vert : vertices)
            {
                if (vert->coords[i] > max_point.coords[i])
                {
                    max_point.coords[i] = vert->coords[i];
                }
            }
        }
        return max_point;
...
{
    // 测试不同的OFF文件
    cout << "测试bunny.off:" << endl;
    poly<double> bunny("bunny.off");
    cout << "顶点数: " << bunny.get_vert_count() << endl;
    cout << "面数: " << bunny.get_facet_count() << endl;
    cout << "表面积: " << bunny.area() << endl;

    auto low = bunny.get_low();
    auto high = bunny.get_high();
    auto center = bunny.get_center();
